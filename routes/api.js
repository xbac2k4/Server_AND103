...
router.post('/add-sinhvien', async (req, res) => {
    try {
        const data = req.body;
        console.log(data)
        const newSinhVien = new SinhVienModel(data);
...
router.put('/update-sinhvien/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const data = req.body;
        console.log(data)
        // const newSinhVien = new SinhVienModel();
        const update = await SinhVienModel.findByIdAndUpdate(id);
        let result = null;
        if (update) {
            update.ma_sinh_vien_ph44315 = data.ma_sinh_vien_ph44315 ?? update.ma_sinh_vien_ph44315,
                update.ten_ph44315 = data.ten_ph44315 ?? update.ten_ph44315,
                update.gioi_tinh_ph44315 = data.gioi_tinh_ph44315 ?? update.gioi_tinh_ph44315,
                update.ngay_sinh_ph44315 = data.ngay_sinh_ph44315 ?? update.ngay_sinh_ph44315,
                update.khoa_hoc_ph44315 = data.khoa_hoc_ph44315 ?? update.khoa_hoc_ph44315,
                update.lop_ph44315 = update.lop_ph44315 ?? update.lop_ph44315,
...
router.delete('/delete-sinhvien/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // const newSinhVien = new SinhVienModel();
...
router.get('/search-sinhvien', async (req, res) => {
    try {
        const { ten_ph44315 } = req.query;
        // const newSinhVien = new SinhVienModel();
        const data = await SinhVienModel.find({
            ten_ph44315: { $regex: new RegExp(ten_ph44315, "i") },
        }).sort({ createdAt: -1 });
...
// router.get('/get-list-xemay', async (req, res) => {
//     try {
//         const data = await XeMayModel.find().populate();
//         res.json({
//             "status": 200,
//             "messenger": "Danh sách xe máy",
//             "data": data
//         })
//     } catch (error) {
//         console.log(error);
...
// router.post('/add-xemay-with-file-image', Upload.single('image'), async (req, res) => {
...
//             ten_xe_ph44315: data.ten_xe_ph44315,
//             mau_sac_ph44315: data.mau_sac_ph44315,
//             gia_ban_ph44315: data.gia_ban_ph44315,
//             mo_ta_ph44315: data.mo_ta_ph44315,
//             hinh_anh_ph44315: urlsImage,

//         }); //Tạo một đối tượng mới
//         const result = (await newXeMay.save()).populate(); //Thêm vào database
...
// });
// // tim kiếm
// router.get('/search-xemay-by-name', async (req, res) => {
//     const {ten_xe_ph44315} = req.query;
//     try {
//         const data = await XeMayModel.find({
//             ten_xe_ph44315: { $regex: new RegExp(ten_xe_ph44315, "i") },
//         }).sort({ createdAt: -1 });
